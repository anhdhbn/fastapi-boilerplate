...
from app.core.dependencies import lang_validator
...
async def fastapi_error_handler(request, exc):
    ...
    return ORJSONResponse(
        status_code=500,
        content=jsonable_encoder(ResponseSchemaBase().custom_response(code, lang.get(code))),
        headers={
            'X-Request-ID': correlation_id.get() or "",
            'Access-Control-Expose-Headers': 'X-Request-ID'
        }
...
async def validation_exception_handler(request: Request, exc: RequestValidationError):
    ...
    return ORJSONResponse(
        ...
        content=jsonable_encoder(get_validation_resp(lang, exc.errors()[0])),
        headers={
            'X-Request-ID': correlation_id.get() or "",
            'Access-Control-Expose-Headers': 'X-Request-ID'
        }
